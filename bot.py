...
# --- ПРЕФИКСЫ ---
def get_prefixes_db():
    return load_data("prefixes.json", {"list": []})

def save_prefixes_db(data):
...
def get_user(user_id):
    ...
    uid = str(user_id)
    # Структура пользователя v2
    default_user = {
        "name": "Неизвестный",
        "username": "None",
        "privilege": RANK_PLAYER,
        "has_beta": False,
        "unlocked_prefixes": [], # Список ID префиксов
        "active_prefix": None    # ID активного префикса
    }
    user = users.get(uid, default_user)
    
    # Миграция старых данных (если остались)
    if "role" in user:
        user["privilege"] = RANK_PLAYER
        user["has_beta"] = False
        del user["role"]
        
    # Принудительная роль владельца
    if user_id == OWNER_ID:
...
async def choose_prefix_menu(callback: types.CallbackQuery):
    user = get_user(callback.from_user.id)
    prefixes_db = get_prefixes_db()["list"]
...
async def show_games_list(callback: types.CallbackQuery):
    ...
    is_vip = user["privilege"] in [RANK_VIP, RANK_ADMIN, RANK_OWNER]
...
async def download_backup(callback: types.CallbackQuery):
    ...
    try:
        if os.path.exists("users.json"): 
            await bot.send_document(callback.message.chat.id, FSInputFile("users.json"), caption="👥 База пользователей")
        if os.path.exists("games.json"): 
            await bot.send_document(callback.message.chat.id, FSInputFile("games.json"), caption="🎮 База игр")
        if os.path.exists("logs.json"): 
            await bot.send_document(callback.message.chat.id, FSInputFile("logs.json"), caption="📜 Логи")
        if os.path.exists("prefixes.json"):
            await bot.send_document(callback.message.chat.id, FSInputFile("prefixes.json"), caption="🏷 Префиксы")
...
async def admin_games_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("🎮 <b>Редактор игр:</b>\nНажми на игру, чтобы изменить.", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def edit_game_menu(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="👁 Тип доступа", callback_data=f"cycleaccess_{idx}")],
...
async def cycle_game_access(callback: types.CallbackQuery):
    ...
    await callback.answer(f"Доступ изменен на {new}")
